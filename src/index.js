...
if (process.env.CITYLAB === 'ALL') {
  ...
  const folder = './keys/'
  privateKey = fs.readFileSync(folder + 'server-key.pem', 'utf8')
  certificate = fs.readFileSync(folder + 'server-cert.pem', 'utf8')
  ca = fs.readFileSync(folder + 'ca-cert.pem', 'utf8')
  credentials = {
    key: privateKey,
    cert: certificate,
    ca: ca
